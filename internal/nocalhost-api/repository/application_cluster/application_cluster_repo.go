...
func (repo *applicationClusterRepo) GetJoinCluster(
	...
	err := repo.db.Table("applications_clusters as ac").
		Select("count(ac.id) as dev_space_count,ac.cluster_id,ac.application_id,c.name as cluster_name,"+
			"c.info as cluster_info,min(ac.created_at) as created_at,if(c.info is null,\"0\",\"1\") as cluster_status").
