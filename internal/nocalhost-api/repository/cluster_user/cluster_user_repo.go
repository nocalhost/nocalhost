...
func (repo *clusterUserRepo) UpdateKubeConfig(
	...
	affect := repo.db.Model(&model.ClusterUserModel{}).
		Where("id=?", models.ID).
		Update("kubeconfig", models.KubeConfig).
		RowsAffected
	if affect > 0 {
		return models, nil
	}
...
func (repo *clusterUserRepo) DeleteByWhere(
	ctx context.Context,
	models model.ClusterUserModel,
) error {
...
func (repo *clusterUserRepo) Update(
	ctx context.Context,
	models *model.ClusterUserModel,
) (*model.ClusterUserModel, error) {
...
func (repo *clusterUserRepo) GetList(
	ctx context.Context,
	models model.ClusterUserModel,
) ([]*model.ClusterUserModel, error) {
...
func (repo *clusterUserRepo) ListByUser(
	ctx context.Context,
	userId uint64,
) ([]*model.ClusterUserPluginModel, error) {
	result := make([]*model.ClusterUserPluginModel, 0)

	repo.db.Raw("SELECT * FROM clusters_users WHERE user_id = ? ORDER BY user_id, id", userId).
		Scan(&result)
	if len(result) > 0 {
		return result, nil
	}
...
func (repo *clusterUserRepo) GetFirst(
	ctx context.Context,
	models model.ClusterUserModel,
) (*model.ClusterUserModel, error) {
...
func (repo *clusterUserRepo) Create(
	ctx context.Context,
	model model.ClusterUserModel,
) (model.ClusterUserModel, error) {
	err := repo.db.Create(&model).Error
	if err != nil {
		return model, errors.Wrap(
			err,
