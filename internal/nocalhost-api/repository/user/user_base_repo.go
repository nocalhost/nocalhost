...
func (repo *userBaseRepo) GetUserList(ctx context.Context) ([]*model.UserList, error) {
	...
	repo.db.Raw("select u.id as id,u.name as name,u.email as email," +
		"count(distinct cu.id) as cluster_count,u.status as status, " +
		"u.is_admin as is_admin from users as u left join clusters_users as cu " +
		"on cu.user_id=u.id where u.deleted_at is null and cu.deleted_at is null group by u.id").
		Scan(&result)
	return result, nil
}
