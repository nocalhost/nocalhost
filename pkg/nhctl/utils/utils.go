...
func KillSyncthingProcessOnWindows(keyword string) {
	...
	log.ErrorE(err, fmt.Sprintf("execute wmic command, keyword: %s", keyword))
	if err != nil {
		return
	}
		...
		if strings.Contains(item, keyword) {
			for _, segment := range strings.Split(item, " ") {
				if pid, err1 := strconv.Atoi(segment); err1 == nil {
					...
					log.WarnE(err, fmt.Sprintf("terminate syncthing pid: %v", pid))
				}
			}
		}
	}
...
func KillSyncthingProcessOnUnix(keyword string) {
	...
	err := command.Run()
	log.WarnE(err, fmt.Sprintf("kill syncthing process, keyword: %s, command: %v", keyword, command.Args))
}
