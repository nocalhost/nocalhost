...
func Create(c *gin.Context) {
	...
	// 1. check if Namespace nocalhost-reserved already exist,
	//	  ignore cause by nocalhost-dep-job installer.sh has checkout this condition and will exit
	...
	cluster, err := service.Svc.ClusterSvc().
		Create(c, req.Name, string(DecKubeconfig),
			req.StorageClass, t.Clusters[0].Cluster.Server, clusterInfo, userId.(uint64))
	if err != nil {
