...
func UpdateResourceLimit(c *gin.Context) {
	...
	if !flag {
		log.Errorf(
			"update devspace resource limit fail. Incorrect resource limit parameter  [ %v ] format.",
			message,
		)
	...
	clusterDevsSetUp.DeleteResourceQuota(resourceQuotaName, devspace.Namespace).
		CreateResouceQuota(resourceQuotaName, devspace.Namespace, req.SpaceReqMem,
