...
func installBookInfoRawManifest() {
	cmd := "nhctl install bookinfo -u https://github.com/nocalhost/bookinfo.git -t rawManifest -n test" +
		" --resource-path manifest/templates --kubeconfig " + util.CODING
...
func installBookInfoHelmGit() {
	cmd := "nhctl install bookinfo -u https://github.com/nocalhost/bookinfo.git -t helmGit  -n test" +
		"  --resource-path charts/bookinfo --kubeconfig=" + util.CODING
	if ok, _ := util.WaitForCommandDone(cmd); !ok {
...
func installBookInfoKustomizeGit() {
	cmd := "nhctl install bookinfo -u https://github.com/nocalhost/bookinfo.git -t kustomizeGit -n test  " +
		"--resource-path kustomize/base --kubeconfig=" + util.CODING
	if ok, _ := util.WaitForCommandDone(cmd); !ok {
